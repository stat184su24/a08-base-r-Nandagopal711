...
```{r}
filtered_data <- iris[iris$Petal.Width > 1, ]
avg_sepal_len <- mean(filtered_data$Sepal.Length)

avg_sepal_len
```
Thus, the average sepal length for flowers with petal widths greater than 1 is 6.325806.
...
```{r}
Counting <- matrix(data = NA, nrow= 3, ncol =4)
...
```{r}
set.seed(184)
plotNormal <- function(repititions, mean, sd){
  rand_num <- rnorm(repititions, mean, sd)
  hist(rand_num)
}
...
plotNormal <- function(repitions, mean, sd, color) {
  rand_num <- rnorm(repitions, mean, sd)
  hist(rand_num, 
       main = paste("Histogram from a Normal(", mean, ", ", sd, ") Distribution", sep = ""),
       col = color)
}
